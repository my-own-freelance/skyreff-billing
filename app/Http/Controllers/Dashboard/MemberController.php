...
class MemberController extends Controller
{
    ...
    {
        try {
            $query = User::with('area:id,name') // relasi area
                ->select('id', 'name', 'username', 'phone', 'is_active', 'area_id', 'last_login_at', 'link_maps', 'address')
                ->where('role', 'member');
            ...
            $output = $data->map(function ($item) use ($user) {
                ...
                $item['last_login_at_formatted'] = $item->last_login_at ? Carbon::parse($item->last_login_at)->locale('id')->translatedFormat('d M Y H:i') : '-';
                $item['action'] = $action;
    ...
    public function update(Request $request)
    {
        try {
            ...
            $rules = [
                'id' => 'required|integer',
                'name' => 'required|string',
                'phone' => 'required|string|digits_between:10,15|unique:users,phone,' . $data['id'] . ',id', // validasi duplikat no telpon dengan user yg beda
                'password' => 'nullable',
                'is_active' => 'required|string|in:Y,N',
                'address' => 'nullable|string|max:255',
                'link_maps' => 'nullable|url',
            ...
            $user = User::where('role', 'member')->where('id', $data['id'])->first();
